...
class WanPipeline(DiffusionPipeline, WanLoraLoaderMixin):
    ...
    def __init__(
        ...
        self.parallel_run = int(os.getenv("WORLD_SIZE", "1")) > 1
    ...
    def _get_t5_prompt_embeds(
        ...
        model_device = self._module_device(self.text_encoder)
        ...
        prompt_embeds = self.text_encoder(
            text_input_ids.to(model_device), mask.to(model_device)
        ).last_hidden_state
    ...
    def decode_next_latent(self, output_type="np"):
        ...
        if self._decode_state["par_vae_decode"]:
            ...
            decode_vae_device = getattr(
                self, "decode_vae_device", getattr(self, "vae_device", self._module_device(self.vae))
            )
            if self._module_device(self.vae) != decode_vae_device:
                self.vae.to(decode_vae_device)
            current_latent = current_latent.to(
                device=decode_vae_device, dtype=self.vae.dtype
            )
            latents_mean = latents_mean.to(current_latent.device, current_latent.dtype)
            latents_std = latents_std.to(current_latent.device, current_latent.dtype)
            current_latent = current_latent / latents_std + latents_mean
    ...
    def __call__(
        ...
        if chunk_i == 0:
            ...
            transformer_device = getattr(
                self, "transformer_device", self._execution_device
            )
            text_encoder_device = getattr(
                self, "text_encoder_device", transformer_device
            )
            vae_device = getattr(self, "vae_device", transformer_device)
            ...
            self.points_local = generate_points_in_sphere(50000, 8.0).to(
                transformer_device
            )
            if self.parallel_run is False:
                print(
                    f"[WAN PIPE] devices: transformer={transformer_device}, "
                    f"text_encoder={text_encoder_device}, vae={vae_device}"
                )

            # 3. Encode input prompt
            print("[WAN PIPE] encode_prompt start")
            ...
            print("[WAN PIPE] encode_prompt done")
            ...
            if not few_step:
                self.scheduler.set_timesteps(
                    num_inference_steps, device=transformer_device
                )
            ...
            if image_path:
                print("[WAN PIPE] image encode start")
                first_image = self.resize_center_crop_to_tensor(image_path, (width, height)) # (B C T H W)
                first_image = first_image.to(vae_device).to(self.vae.dtype)
                ...
                latents_mean = (
                    torch.tensor(self.vae.config.latents_mean)
                    .view(1, self.vae.config.z_dim, 1, 1, 1)
                    .to(first_image_condition.device, first_image_condition.dtype)
                )
                latents_std = 1.0 / torch.tensor(self.vae.config.latents_std).view(
                    1, self.vae.config.z_dim, 1, 1, 1
                ).to(first_image_condition.device, first_image_condition.dtype)
                first_image_condition = (
                    (first_image_condition - latents_mean) * latents_std
                ).to(transformer_device, latents.dtype)
                print("[WAN PIPE] image encode done")


            self.use_kv_cache = False
            self.init_kv_cache()
            print("[WAN PIPE] diffusion loop start")
        ...
        selected_frame_indices_override = self.ctx.get("selected_frame_indices_override")
        if chunk_i == 0:
            ...
            if selected_frame_indices_override is not None:
                selected_frame_indices = list(selected_frame_indices_override)
            elif (
                use_memory
        ...
        with self.progress_bar(total=4) as progress_bar:
            for i, t in enumerate(timesteps[:-1]):
                ...
                if chunk_i > 0 and i == 0 and self.use_kv_cache:
                    latents_cache = latents_curr[:, :, selected_frame_indices]
                ...
                latent_model_input = latents_curr[:, :, -generate_latent_num:].to(transformer_dtype)
        ...
        if not output_type == "latent":
            if not self.par_vae_decode:
                latents = latents.to(
                    device=getattr(self, "vae_device", self._module_device(self.vae)),
