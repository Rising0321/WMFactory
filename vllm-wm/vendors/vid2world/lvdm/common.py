...
def extract_into_tensor_2d(a, t, x_shape):
    """
    Extract values from 1D tensor a based on indices t, supports t being 2D tensor [B,T].
    
    Args:
        a: n-dim==1 tensor
        t: [B,T]
        x_shape: target tensor shape
    
    Returns:
        suitable to broadcast with x_shape
    """
    if len(t.shape) == 1:
        b = t.shape[0]
        out = a.gather(-1, t)
        ...
        flat_t = t.reshape(-1)
        flat_values = a.gather(-1, flat_t)
        values_bt = flat_values.reshape(b, t_dim)
        
        output_shape = [b]
        for i in range(1, ndim):
            if i == time_dim_idx:
                output_shape.append(t_dim)
            else:
