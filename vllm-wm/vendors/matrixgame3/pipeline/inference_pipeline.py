...
class MatrixGame3Pipeline:
    ...
    def _move_vae(self, device):
        if self.rank != 0 or self.vae is None:
            return
        self.vae.device = torch.device(device)
        self.vae.model.to(device=self.vae.device, dtype=self.vae.dtype)
        if getattr(self.vae, "encoder_model", None) is not None:
            self.vae.encoder_model.to(device=self.vae.device, dtype=self.vae.dtype)
        self.vae.scale[0] = self.vae.scale[0].to(device=self.vae.device, dtype=self.vae.dtype)
        self.vae.scale[1] = self.vae.scale[1].to(device=self.vae.device, dtype=self.vae.dtype)

    def _single_gpu_offload(self, target):
        if dist.is_initialized():
            return
        if target == "vae_encode":
            self.model.to("cpu")
            if not self.t5_cpu:
    ...
    def generate(self,
        ...
        if not dist.is_initialized():
            if not self.t5_cpu:
                self.text_encoder.model.to("cpu")
            self._single_gpu_offload("vae_encode")
        ...
        if not dist.is_initialized():
            self._single_gpu_offload("diffusion")
        ...
        with torch.no_grad():
            ...
            for clip_idx in range(num_iterations):
                ...
                if self.use_async_vae:
                    ...
                    if self.rank == 0:
                        if not dist.is_initialized():
                            self._single_gpu_offload("vae_decode")
                        vae_profiler = {}
                        ...
                        if not dist.is_initialized() and clip_idx != num_iterations - 1:
                            self._single_gpu_offload("diffusion")
