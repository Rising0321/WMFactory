...
def get_current_action():
    ...
    def _parse_mouse(raw: str):
        raw = raw.strip().lower()
        if not raw:
            return None
        if "," in raw:
            try:
                pitch_str, yaw_str = raw.split(",", 1)
                return [float(pitch_str), float(yaw_str)]
            except Exception:
                return None
        ...
        yaw = 0.0
        for ch in raw:
            if ch not in CAMERA_VALUE_MAP:
                return None
            delta_pitch, delta_yaw = CAMERA_VALUE_MAP[ch]
            pitch += float(delta_pitch)
            yaw += float(delta_yaw)
        return [pitch, yaw]

    def _parse_keyboard(raw: str):
        raw = raw.strip().lower()
        if not raw:
            return None
        if raw == "q":
            return [0, 0, 0, 0, 0, 0]
        if any(ch not in "wsad" for ch in raw):
            return None

        pressed = {"w": 0, "s": 0, "a": 0, "d": 0}
        for ch in raw:
            pressed[ch] = 1

        if pressed["w"] and pressed["s"]:
            pressed["w"] = 0
            pressed["s"] = 0
        if pressed["a"] and pressed["d"]:
            pressed["a"] = 0
            pressed["d"] = 0

        return [pressed["w"], pressed["s"], pressed["a"], pressed["d"], 0, 0]
    ...
    while flag != 1:
        try:
            ...
            parsed_mouse = _parse_mouse(idx_mouse)
            parsed_keyboard = _parse_keyboard(idx_keyboard)
            if parsed_mouse is not None and parsed_keyboard is not None:
                flag = 1
        except:
            pass
    mouse_cond = torch.tensor(parsed_mouse)
    keyboard_cond = torch.tensor(parsed_keyboard)
