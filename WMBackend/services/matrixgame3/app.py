...
class MatrixGame3RuntimeService:
    def __init__(self) -> None:
        ...
        self.python_bin = os.getenv(
            ...
            os.getenv("WM_VLLM_WM_PYTHON", sys.executable),
        )
        self.ckpt_dir = Path(
            os.getenv(
                "WM_MATRIXGAME3_CKPT_DIR",
                str(MATRIXGAME3_CHECKPOINT_ROOT),
            )
        ).resolve()
        self.output_dir = Path(
            os.getenv("WM_MATRIXGAME3_OUTPUT_DIR", str(PROJECT_ROOT / "outputs" / "matrixgame3" / "frontend"))
        ).resolve()
