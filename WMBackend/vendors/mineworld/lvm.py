...
def _compile_enabled():
    enable = os.getenv("MINEWORLD_ENABLE_COMPILE", "0").lower()
    if enable not in ("1", "true", "yes", "on"):
        return False
    disable = os.getenv("TORCH_COMPILE_DISABLE", "0").lower()
    return hasattr(torch, "compile") and disable not in ("1", "true", "yes", "on")
...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        if kv_len is None:
            kv_len = int(position_ids.max().item()) + 1
        key_states, value_states = (
                self.cache_k[:bsz, :kv_len, :],
                self.cache_v[:bsz, :kv_len, :],
            )
...
class LlamaForCausalLM(PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.prefill_fn = self._maybe_compile(prefill, fullgraph=True, dynamic=True)
        self.decode_one_token_fn = self._maybe_compile(decode_one_token, mode="max-autotune", fullgraph=True)
        self.decode_some_token_fn = self._maybe_compile(decode_some_token, mode="max-autotune", fullgraph=True)
        self.post_init()      

    def _maybe_compile(self, fn, **kwargs):
    ...
    def naive_generate(self, input_ids, max_new_tokens, temperature=1.0, action_all=None, top_p=None, top_k=None):
        ...
        generated_tokens = decode_n_tokens(
            ...
            temperature = temperature,
            decode_one_token_function=self.decode_one_token_fn,
            action=action_all,
            top_p = top_p,
            top_k = top_k,
        )
    ...
    def diagd_img_token_for_gradio(self, input_action, position_id, max_new_tokens, temperature=1.0, windowsize=2):
        ...
        with torch.no_grad(), torch.autocast("cuda", dtype=torch.float16):
            generated_tokens, position_id = img_diagd_decode_n_token_for_gradio(
                ...
                num_generate_tokens = max_new_tokens,
                temperature = temperature,
                decode_some_token_function=self.decode_some_token_fn,
