...
`WMBackend` is a standalone source project for interactive world models.
...
At runtime, `WMBackend` should not need the outer `WMFactory/models/*` or `WMFactory/services/*` trees.
...
`WMBackend` uses one orchestrator plus one worker subprocess per model.
